...
import { LayoutDashboard, Map, Activity, Settings, Bell, Zap, Wind, AlertTriangle } from 'lucide-react';
...
const healthData = [
  ...
  { time: '15:00', health: 85, predicted: 80 }, // Dipping
];

const Sidebar = () => (
  <div className="w-20 bg-gray-900 border-r border-gray-800 flex flex-col items-center py-8 space-y-8 h-screen">
...
const NavItem = ({ icon, active, alert }) => (
  <div className={`relative p-3 rounded-xl cursor-pointer transition-all ${active ? 'bg-gray-800 text-blue-400' : 'text-gray-500 hover:bg-gray-800 hover:text-gray-300'}`}>
    {icon}
    {alert && <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full"></span>}
  </div>
);
    ...
    <div className="flex h-screen bg-black text-white font-sans overflow-hidden">
      ...
      <div className="flex-1 flex p-6 gap-6">
        ...
        <div className="w-1/3 flex flex-col gap-6">
           {/* Header */}
          <div className="bg-gray-900 rounded-2xl p-6 border border-gray-800 shadow-2xl h-full relative overflow-hidden group">
            ...
            {/* 3D Model Placeholder (Where AWS TwinMaker goes) */}
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-b from-gray-800 to-gray-900 relative">
              {/* Model Viewer (replace with your GLB at public/models/telecom-tower.glb) */}
        ...
        <div className="w-2/3 flex flex-col gap-6">
          ...
          <div className="h-1/2 bg-gray-900 rounded-2xl border border-gray-800 shadow-lg relative overflow-hidden">
            ...
            <div className="w-full h-full bg-gray-800 flex items-center justify-center">
               <p className="text-gray-500">Map Component (Leaflet / Google Maps)</p>
               {/* Simulation of a red dot selected */}
               <div className="absolute top-1/2 left-1/2 w-4 h-4 bg-red-500 rounded-full shadow-[0_0_20px_rgba(239,68,68,0.6)] cursor-pointer ring-4 ring-red-900/50"></div>
               <div className="absolute top-1/3 left-1/4 w-3 h-3 bg-green-500 rounded-full opacity-60"></div>
               <div className="absolute bottom-1/3 right-1/3 w-3 h-3 bg-green-500 rounded-full opacity-60"></div>
            </div>
          </div>
